...
** If an id is present in one or more GDCs, and the metadata compliant with the WCMP2 latest approved version of WCMP2 and missing from one or more GDCs, the SCGDC will increase the metric ``wmo_wis2_scgdc_records_incorrect_total`` by ``1`` for all GDCs missing the id.

...
In order to assess the downloading feature of a Global Cache, and to provide the required information to determine the KPI on downloads, the Sensor Centre SHALL:

* Select randomly every minute a sample of at least 5 data granules successfully cached by all Global Cache. 
* Increase by the number of selected data granule the metric ``wmo_wis2_scgc_download_selected_total``
* Attempt a download for that same granule towards all Global Cache.
* Increase by 1 ``wmo_wis2_scgc_download_successful_total`` if the download is succesful.

...
* If data is not cached on a Global Cache for any reason (that is, if the data is sent to the ``data/recommended`` topic, if the data is flagged as ``cache=false`` or if there is a problem with the data set), the Global Cache shall still republish the WIS2 Notification Message to the ``cache/a/wis2`` topic. In this case the message should not be modified.

...
* For messages received on topic ``origin/a/+/data/core/#`` a Global Cache shall:
** If the message contains the property ``"properties.cache": false``
*** Republish the unmodified message at topic ``cache/a/wis2/...`` matching ``origin/a/wis2/...`` where the original message has been received.
** else
...
**** Modify the message identifier, the canonical link's ``href`` of the received message. Leave all other fields untouched.
**** Republish the modified message to topic ``cache/a/wis2/...`` matching the ``origin/a/wis2/...`` where the original message has been received.

...
** A Global Cache will attempt to download the resource from the URL provided in the ``href`` property of the link object with link relation ``canonical`` (or link relation ``update`` for updated resources). If present, a Global Cache may attempt to cache content embedded in the ``properties.content`` object of the notification message. 
...
** A Global Cache will publish WIS2 Notification Messages advertising the availability of each resource that it has cached. The notification messages will follow the standard structure (see _Manual on WIS_, Volume II -Appendix E. WIS2 Notification Message).
** If the Global Cache receives a WIS2 Notification Message containing the property ``"properties.cache": false``, the Global Cache will not attempt to cache this resource. It will immediately publish a new notification message on the ``cache/a/wis2/...`` topic with links the point to the original location of the resource - the resource is not cached, so there are no new URLs to update the links. 
** Errors will result in the notification message being discarded.
...
* A notification message published by a Global Cache will be a near-identical copy of the original notification message it received, with the following exceptions:
** a new unique message ID will be used;
** the canonical links be updated to point to where the resource is now cached; 
** Global Cache specific properties may be added.
...
* A Global Cache will add the ``properties.global-cache`` property (with its centre identifier as the value) in notification messages that it publishes to ``cache/a/wis2/{centre-id}``.

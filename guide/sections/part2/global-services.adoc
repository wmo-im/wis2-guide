...
** JSON
* A Global Replay shall subscribe to a Global Broker instance, to the topics `+origin/a/wis2/#+`, `+cache/a/wis2/#+` and `+monitor/a/wis2/#+`.
...
*** `subscriber-id`: a unique identifier (UUID) defined by the user/client.
*** `topic-id`: the topic on which to trigger message replication (centre-id required, allowing `+` and `#` for subsequent topic levels)
...
===== 2.7.6.2 Global Replay Reference Implementation: wis2-grep
...
==== 2.7.7 Global Monitor

===== 2.7.7.1 Technical considerations

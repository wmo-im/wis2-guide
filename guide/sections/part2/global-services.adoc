...
In WIS2 Global Caches provide access to WMO Core Data for Data Consumers. This allows for Data Providers to restrict access to their system and it mitigates the need for high bandwith and low latency access to data from the Data Providers. Global Caches work transparent for End Users in that they resend notification messages from Data Providers which are updated to point to the Global Caches data store for data, they copied from the original source. From a Data Consumer perspective they SHOUD subscribe to the notification messages from Global Caches instead of the notification messages from the Data Providers and follow the URLs from that messages which either point to a Cache holding the data, or - in case of recommended data or uncacheable content - point to the original source.
...
* A Global Cache is designed to support real-time distribution of content. Data Consumers access data objects from a Global Cache instance by resolving the URL in a "data availability" notification message and downloading the file. Apart from the URL it is transparent to the Data Consumers from which Global Cache they download the data. There is no need to download the same Data Object from multiple Global Caches.
...
** If the message contains Cache: false
...
**** Modify *only* the href and the topic of the received message. Leave all other fields untouched. This holds especially for the content field, the pubtime, the data_id and datetime
**** Republish the message at topic */cache/a/wis2/*
*** else

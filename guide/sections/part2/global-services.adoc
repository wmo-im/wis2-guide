...
** If the Global Cache receieves a WIS2 Notification Message containing the property ``"properties.cache": false``, the Global Cache will not attempt to cache this resource. It will immediately publish a new notification message on the ``cache/a/wis2/...`` topic with links the point to the original location of the resource - the resource is not cached, so there are no new URLs to update the links. 

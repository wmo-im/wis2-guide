...
It is up to Data Publishers to decide how they group data into Datasets. That said, we recommend that, subject to the consistency rules, Data Publishers should organise their data into as few Datasets as possible. For a Data Publisher this means fewer discovery metadata records to maintain. For a Data Consumer this means fewer topics to subscribe to, and low barrier APIs to access.
